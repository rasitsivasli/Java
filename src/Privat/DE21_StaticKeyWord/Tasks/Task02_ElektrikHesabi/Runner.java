package Privat.DE21_StaticKeyWord.Tasks.Task02_ElektrikHesabi;

public class Runner {
    /*
    Task 02 ->
    ElektirikHesap Class:  fields : toplamTuketim(int), oran(double), fatura(double)
    Müşteriye ait tüketilen enerjiyi toplamTuketim'e toplayan tüketimEkle method
    oran(0.7)  katsaysı ile  toplamTuketim çarparak fatura değeri atayan odenecekTutar method

    Müsteri Class: fields: name ElektrikHesabi class obj.
    B_RunnerMusteri Class obj ile enz 2  aylık elektrik tüketim faturası print eden code create ediniz

     */

    public static void main(String[] args) {

        Musteri name = new Musteri("Rasit Sivasli");
        ElektrikHesabi toplamTuketim = new ElektrikHesabi();
        System.out.println(name.name+" adli müsterinin 2 aylik elektrik faturasi toplam : "+toplamTuketim.elctrikHesaplama());

    }




}
